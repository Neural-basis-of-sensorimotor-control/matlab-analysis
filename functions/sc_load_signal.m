function [signal, file, expr] = sc_load_signal(experiment_file, file_str, ...
	signal_str, varargin)
...
if isa(experiment_file, 'struct')
	...
	elseif nargin==2
		signal = sc_load_signal([sc_dir neuron.expr_file], neuron.file_str, ...
			neuron.signal_str, file_str);
	elseif nargin==3
		signal = sc_load_signal([sc_dir neuron.expr_file], neuron.file_str, ...
			neuron.signal_str, file_str, signal_str);
	else
		signal = sc_load_signal([sc_dir neuron.expr_file], neuron.file_str, ...
			neuron.signal_str, file_str, signal_str, varargin{:});
	end
	return
end
