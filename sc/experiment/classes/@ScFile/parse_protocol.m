...
if obj.is_adq_file
  ...
  while ~is_end_of_file(line) && (isempty(line) || is_comment(line)) ....
      && ~is_start_of_sequence(line)
  ...
  while ~is_end_of_file(line) && ~(isempty(line) || is_comment(line))
    ...
    triggertimes(line_count) = str2double(line);
    line                     = fgetl(fid);
    
  end
  
  triggertimes   = triggertimes(1:line_count);
  ...
  while ~is_end_of_file(line) && ~is_start_of_sequence(line)
    line = fgetl(fid);
  end
  
  if ~is_end_of_file(line) && is_start_of_sequence(line)
...
function ret = is_event(line)

ret = length(line) > 2 && strcmpi(line(end-1:end),'##');

end


% Return true if line contains a new neuron
function ret = is_new_neuron(line)

ret = length(line) > 2 && strcmpi(line(1:2),'%%');

end


function ret = is_end_of_file(line)
...
function ret = is_start_of_sequence(line)
