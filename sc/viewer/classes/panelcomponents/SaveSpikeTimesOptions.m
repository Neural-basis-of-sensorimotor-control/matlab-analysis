  ...
  methods (Access = 'protected')
    ...
    function save_spiketimes_callback(obj)
      ...
      function okbtn_callback(~,~)
        ...
        headers_formatted = cellfun(@(x) ['"' strrep(x, '"', '''') '"'], ...
          headers, 'UniformOutput', false);
        
        for i=1:numel(headers_formatted)-1
          fprintf(fid, '%s,', headers_formatted{i});
        end
        
        fprintf(fid,'%s\n', headers_formatted{end});
        alltimes = cell(numel(headers_formatted));
